...
### External Interfaces
* **AXI-Lite (`IF_s_axireg`)**: Configuration registers controlled by Python software (PYNQ).
* **DMA AXI-Stream (`s_dma_axis`, `m_dma_axis`)**: High-speed memory loading/unloading interface.
* **Input Ports (`port_tdata_i`, `port_tvalid_i`)**: Streaming inputs from ADCs and digital readouts.
* **Output Ports (`port_tdata_o`, `port_tvalid_o`, `m_axis_tdata`)**: Data and waveform parameters sent to the RF Signal Generators.
...
* It continuously monitors `time_abs`. When the absolute time matches the pulse request timestamp, it forwards the AXI-Stream packet to the external Signal Generators, guaranteeing phase coherence regardless of CPU latency.

---

## 3. Signal Naming Conventions Refresher
