...
assign stall   = bubble_id | bubble_rd | id_wait_stall;
...
wire [ 15 : 0 ] if_op_code ;              // zpx: the header
wire [ 55 : 0 ] if_op_data ;              // zpx: the payload
...
always_comb begin : DECODER
   // zpx: id_SO = 1: wave memory/register, id_TO = 1: data memory/register
   ...
   id_type_wait      = ( id_HEADER == WAIT    ) ;
   id_type_wra       = id_type_wr & (~id_SO & ~id_TO)  ; // Write Data Register from ALU
   id_type_wrd       = id_type_wr & ~(id_SO & ~id_TO)  ; // Write Data Register (ALU, MEM, or IMM) zpx:write back but not write to wave register
   ...
   // zpx: id_ here means final decision (internal control signals)
   // zpx: cfg_ here means mediate signals
   // zpx: dual means "external operation"
   ...
   // zpx: 9-bit signal: [8] is enable, [7] is type (0:int, 1:ext), [6:0] is command content
   ...
   id_wait_stall     = id_type_wait & wait_stall;
end
...
// zpx: can compare with "_qproc_defines.svh" file to get each signas's meaning
...
assign id_ctrl.cfg_dt_imm    = id_cfg_dt_imm    ;     //zpx: decide the data source written into MEM (from ALU or immediate)
assign id_ctrl.cfg_port_src  = id_cfg_port_src  ;     //zpx: write data from Wave Memory(=0), write data from Internal Register(=1)
assign id_ctrl.cfg_port_type = id_type_wpd      ;     //zpx: decide Data Port(=1) or Waveform Port(=0)
assign id_ctrl.cfg_port_time = id_type_wp & id_TO ;
assign id_ctrl.cfg_cond      = id_COND          ;
assign id_ctrl.cfg_alu_src   = id_cfg_alu_src   ;     //zpx: decide ALU source (from ALU or 24bits immediate), 32bits immediate will bypass ALU
assign id_ctrl.cfg_alu_op    = id_cfg_alu_op    ;
assign id_ctrl.usr_ctrl      = id_usr_ctrl      ;     // zpx: custom remote control signals for internal/external devices
...
always_comb begin
   ...
   // zpx: "write wave registers" or "write wave memory" or "write port" is unconditionally executed
...
assign rd_stage_zero = (bubble_id | id_wait_stall) & !bubble_rd ;
...
assign usr_en_o        = halt ? 0 : x1_ctrl.usr_ctrl[8] ; // zpx: enable bit from usr_ctrl[8]
assign usr_ctrl_o      = halt ? 0 : x1_ctrl.usr_ctrl[7:0] ; // zpx: 8-bit command (bit 7 type + bits 6:0 content)
